...
from implementation.answer_gemini import answer_question, fetch_context_unranked
...
def evaluate_answer(test: TestQuestion, fast: bool = True) -> tuple[AnswerEval, str, list]:
    """
    Evaluate answer quality using LLM-as-a-judge (Ollama qwen3:4b).
    ...
    """
    generated_answer, retrieved_docs = answer_question(test.question,fast=True)
    ...
    judge_messages = [
        {
            ...
            "content": (
                ...
                "\"feedback\" (string, your evaluation comments), "
                "\"accuracy\" (float 1-5), \"completeness\" (float 1-5), \"relevance\" (float 1-5). "
                "The \"feedback\" key is required. No markdown, no code blocks, raw JSON only."
            ),
        },
        {
            "role": "user",
...
Provide detailed feedback and scores from 1 (very poor) to 5 (ideal) for each dimension. If the answer is wrong, then the accuracy score must be 1.""",
    ...
    judge_response = ollama_client.chat(
        ...
        format=AnswerEval.model_json_schema(),
        options={"temperature": 0},
    )
    raw = judge_response.message.content
    if "<tool_call>" in raw:
       raw = raw.split("<tool_call>")[-1].strip()
    answer_eval = AnswerEval.model_validate_json(raw)
...
def run_cli_evaluation(test_number: int):
    """Run evaluation for a specific test by row number."""
    ...
    answer_result, generated_answer, retrieved_docs = evaluate_answer(test,fast=True)
